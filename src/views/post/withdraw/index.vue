<template>
  <div class="app-container">
    <eHeader :query="query"/>
    <!--表格渲染-->
    <el-table v-loading="loading" :data="data" :height="tableHeight" size="small" style="width: 100%;">
      <!--<el-table-column prop="bizId" label="bizId"/>
      <el-table-column prop="orderNo" label="流水号" width="250px"/>
     ...
     <el-table-column prop="ucoinUrl" label="U币地址" width="100px"> 
      ...
      <el-table-column prop="alipayId" label="支付宝ID" width="150px"/>
      <el-table-column prop="alipayName" label="支付宝姓名"/>
      <el-table-column prop="accountNo" label="银行卡号" width="200px"/>
      <el-table-column prop="accountName" label="银行卡姓名" width="200px"/>
      <el-table-column prop="accountBankName" label="银行开户行"  width="150px"/>      
      <el-table-column prop="amt" label="提现金额"  width="150px">
       <template slot-scope="scope"> 
      ...
      <el-table-column fixed="right" v-if="checkPermission(['ADMIN','FLOWWITHDRAW_ALL','FLOWWITHDRAW_EDIT','FLOWWITHDRAW_DELETE'])" label="操作" width="250px" align="center">
            <template slot-scope="scope">
            <el-button v-if="scope.row.stt =='5'" slot="reference" type="danger" icon="el-icon-delete" @click="withdraw(scope.row)" size="mini" >提现确认</el-button>
            <el-button v-if="scope.row.stt =='5'" slot="reference" type="danger" icon="el-icon-delete"  @click="withdrawCancel(scope.row)" size="mini" >撤销提现</el-button>
...
import { withdraw,withdrawCancel } from '@/api/game/flowWithdraw'
...
export default {
  ...
  methods: {
    ...
    withdrawCancel(row) {
      ...
      withdrawCancel(row.orderNo).then(res => {
